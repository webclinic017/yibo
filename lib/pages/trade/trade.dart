...
class _trade extends State<trade> {
  ...
  void initState() {
    ...
    weituo_way.add(DropdownMenuItem(value: '1', child: Container(padding:EdgeInsets.only(left: 5,right: 130),child: Text('限价委托'),)));
    weituo_way.add(DropdownMenuItem(value: '2', child: Container(padding:EdgeInsets.only(left: 5,right: 130),child: Text('市价委托'),)));
    getTradeData();
    getMyHeYue();
  }
                          ...
                          Container(
                            child: Row(
                              children: <Widget>[
                                ...
                                Text(max_buy_stock_num.toStringAsFixed(0)),
                              ],
                            ),
                          ),
                          Container(
                                                    ...
                                                    Container(
                                                      ...
                                                      child: MaterialButton(
                                                        ...
                                                        onPressed: () async{
                                                          ...
                                                          String stock_code = "sh00001";
                                                          String stock_name = "浦发银行";
                                                          double buy_price = cur_price;
                                                          int trade_direction = int.parse(default_weituo_way);
                                                          int buy_hand = int.parse(number);
                                                          int entrust_way = cur_page;
                                                          ResultData result = await HttpManager.getInstance().post(Address.BASE_URL+"frontend/order/makeOrder",params:{"member_heyue_id":member_heyue_id,"stock_code":stock_code,"stock_name":stock_name,"buy_hand":buy_hand,"buy_price":buy_price,"trade_direction":trade_direction,"entrust_way":entrust_way},withLoading: false);
                                                        },
                                                      ),
                                                    ),
                                                  ],
  ...
  Future getTradeData() async {
    try {
      String result;
      result = await request().send_get('/stock/getTradeData/321');
